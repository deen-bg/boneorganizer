...
<head>
	...
	<title>Servies | B ONE </title>
	<link rel="icon" type="image/png" href="<?=base_url('assets/img/logo/B ONE.png');?>" />
...
<body class="">
			                  		...
			                  		<div class="fixed-table-toolbar">
			                  			<div class="bs-bars pull-left">
			                  				<div class="btn-group hidden-sm-down" id="exampleToolbar" role="group">
							                    <button type="button" class="btn btn-success btn-outline btn-sm" onclick="window.location.href = '<?=base_url('Admin/form_service');?>';">
			                  				...
			                  				<table id="exampleTableToolbar" data-mobile-responsive="true" class="table table-hover" data-pagination="true" data-search="true" style="margin-top: 0px;">
			                    				<thead style="">
			                    					<tr>
														...
														<th style="" data-field="Caption">
			                    							<div class="th-inner ">Caption</div>
			                    							<div class="fht-cell"></div>
			                    						</th>
			                    				...
			                    				<?php if(!empty($servicies)) : ?>
			                  					<tbody>
			                  						<?php 
													$i = 0;
													foreach($servicies as $val_item) : 
													?>
			                  						<tr> 
			                  							<td class=""><?=$i+1; ?></td> 
														  <td style=""><?=$val_item['caption']; ?></td>
			                  							<td style=""><?=$val_item['name']; ?></td>
														<td style="">
															<a type="button" href="<?=base_url('Admin/service_document/'.$val_item['id'].'/?slug='.$val_item['name']);?>" class="btn btn-outline-primary" data-toggle="tooltip" data-placement="top" title="File upload">
														...
														<td style="">
			                  							...
			                  							<td>
			                  								<button type="button" class="btn btn-round btn-warning btn-sm" onclick="window.location.href = '<?=base_url('Admin/edit_service/'.$val_item['id']);?>';"><i class="icon wb-pencil" aria-hidden="true"></i></button>
			                  								<button type="button" class="btn btn-round btn-danger btn-sm" onclick="delServ(<?=$val_item['id'];?>, <?=$val_item['is_active'];?>)"><i class="icon wb-close" aria-hidden="true"></i></button>
			                  							</td> 
			                  						</tr>
			                  						<?php
			                  							$i++;
	...
	<script type="text/javascript">
		...
		function deleteService()
		{
			$.ajax({
				type 	: 'POST',
				url 	: '<?=base_url("Admin/delService"); ?>',
				data 	: {id:$('#serv_id').val(),st:$('#serv_st').val(), action:'delService', '<?=$this->security->get_csrf_token_name(); ?>':'<?=$this->security->get_csrf_hash(); ?>'},
